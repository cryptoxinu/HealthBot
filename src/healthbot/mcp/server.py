...
# Internal field prefixes that must never leak to MCP clients.
_INTERNAL_FIELD_PREFIXES = ("_id", "encrypted_", "raw_")


def _strip_internal_fields(record: dict) -> dict:
    """Remove internal fields from a dict before returning to MCP clients."""
    return {
        k: v for k, v in record.items()
        if not any(k.startswith(prefix) for prefix in _INTERNAL_FIELD_PREFIXES)
    }
...
def create_server(clean_db: CleanDB, phi_firewall: PhiFirewall):
    ...
    def get_lab_results(
        ...
        try:
            ...
            logger.error("MCP get_lab_results failed: %s", exc)
            return f"[Error retrieving lab results: {type(exc).__name__}]"
    ...
    def get_medications(status: str = "active") -> str:
        ...
        try:
            ...
            logger.error("MCP get_medications failed: %s", exc)
            return f"[Error retrieving medications: {type(exc).__name__}]"
    ...
    def get_wearable_data(
        ...
        try:
            ...
            logger.error("MCP get_wearable_data failed: %s", exc)
            return f"[Error retrieving wearable data: {type(exc).__name__}]"
    ...
    def get_health_summary() -> str:
        ...
        try:
            ...
            logger.error("MCP get_health_summary failed: %s", exc)
            return f"[Error retrieving health summary: {type(exc).__name__}]"
        ...
        try:
            ...
            for r in results:
                ...
                if source == "lab":
                    ...
                    lines.append(f"- {source}: {_strip_internal_fields(r)}")

            return _safe_response("\n".join(lines))
        except Exception as exc:
            logger.error("MCP search_health_data failed: %s", exc)
            return f"[Error searching health data: {type(exc).__name__}]"
    ...
    def get_health_trends(metric: str, days: int = 90) -> str:
        ...
        try:
            ...
            logger.error("MCP get_health_trends failed: %s", exc)
            return f"[Error retrieving health trends: {type(exc).__name__}]"
    ...
    def get_hypotheses() -> str:
        ...
        try:
            ...
            logger.error("MCP get_hypotheses failed: %s", exc)
            return f"[Error retrieving hypotheses: {type(exc).__name__}]"
    ...
    def run_skill(skill_name: str) -> str:
        ...
        try:
            ...
            logger.error("MCP run_skill failed: %s", exc)
            return f"[Error running skill '{skill_name}': {type(exc).__name__}]"
