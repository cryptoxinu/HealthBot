...
8. **Key exposure on disk** — Keys never written to files; passphrase-derived only. Note: MCP auto-unlock optionally accepts `HEALTHBOT_PASSPHRASE` as an env var for unattended startup (user-initiated). Setup can optionally store the passphrase in macOS Keychain (`healthbot_mcp_passphrase`) for this purpose — requires explicit user opt-in.
